...
import { authenticate } from "../../middleware/socketAuth.js";
import type { SocketHandler } from "../../sockets/socketServer.js";
import { getAllRooms } from "../rooms/controllers/getRoom.js";
...
export class ChatSocket implements SocketHandler {
    ...
    private async onConnect(socket: Socket) {
        ...
        console.log("CHAT_SOCKET: User Connected", socket.data.user.name)
    ...
    private async onDisconnect(socket: Socket) {
        ...
        console.log("CHAT_SOCKET: User Disconnected", socket.data.user.name)
    ...
    private async syncMessages(socket: Socket, rooms: Room[]) {
        for (const room of rooms) {
            ...
            const fallbackId = `${socket.data.session.createdAt.getTime()}-0`;

            const lastId = await redis.hGet(syncKey, sessionId) ?? fallbackId;
            const messages = await redis.xRange(streamKey, `(${lastId}`, "+");
